...
Build **Ageha** (アゲハ — Japanese for *swallowtail butterfly*), a native desktop manga reader for **Windows** that reproduces the experience of the Android app at `https://github.com/Kotatsu-Redo/Kotatsu-Redo`.
...
2. Produce the icon matrix: `.ico` (Windows, 16–256) and the PNG ladder (16/24/32/48/64/128/256/512) that Conveyor uses. The `.icns` and Freedesktop outputs are still generated by `:tools:brandkit` and are no longer shipped anywhere — Ageha is Windows-only.
...
- Conveyor gives signed delta updates, bundles a jlink-optimised JVM, and needs no code changes. It is used for Windows alone; the multi-platform capability is not exercised. If it ever becomes unavailable, fall back to `jpackage` plus a small custom updater reading a signed JSON manifest from GitHub Releases.
...
- **`release.yml`** — on tag, build and publish the Windows installer (msi/exe) plus the update feed.
